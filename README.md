...
| ✅ | `media-integrity-check/mediaIntegrityCheck.js` | [§3.1.1](https://secureacademic.com/gdpr-architectural-background/#sec-3-1-1) Server-Side Media Validation | A dependency-free container/format sniffer that rejects a renamed video masquerading as audio before it reaches the AI pipeline. Published here before the architecture document described it; §3.1.1 has since been written around it. |
...
- **"The upload never passes through the backend"** — read `signed-url-flow/issue-upload-url.js`: the request body it handles never contains file bytes, only a filename and content type. Note the scope of that claim: it is about the upload path, not about the object being unreachable. The same file names the two bounded cases where the backend does read object content afterwards — the media-integrity check ([§3.1.1](https://secureacademic.com/gdpr-architectural-background/#sec-3-1-1)) and the Proofreader's chunking step ([§5.3](https://secureacademic.com/gdpr-architectural-background/#sec-5-3)).
