...
| ✅ | `signed-url-flow/issue-upload-url.js` | [§3.1](https://secureacademic.com/gdpr-architectural-background/#sec-3-1) / [§5.3](https://secureacademic.com/gdpr-architectural-background/#sec-5-3) Signed URL Architecture | The backend issues a time-limited, write-only GCS URL; the upload never passes through it. The file also states precisely the two bounded cases where the backend *does* read object content afterwards. |
...
| ✅ | `media-integrity-check/mediaIntegrityCheck.js` | [§3.1.1](https://secureacademic.com/gdpr-architectural-background/#sec-3-1-1) Media Integrity Validation | A dependency-free container/format sniffer that rejects a renamed video masquerading as audio before it reaches the AI pipeline. Now runs inside the isolated service below, not the backend — see [`isolated-media-validator/`](./isolated-media-validator/). |
| ✅ | `isolated-media-validator/index.js` | [§3.1.1](https://secureacademic.com/gdpr-architectural-background/#sec-3-1-1) Media Integrity Validation | The Cloud Run request handler that performs the bounded byte read(s) and calls the parser above — entirely outside the backend. |
| ✅ | `isolated-media-validator/call-from-backend.js` | [§3.1](https://secureacademic.com/gdpr-architectural-background/#sec-3-1) / [§3.1.1](https://secureacademic.com/gdpr-architectural-background/#sec-3-1-1) | The backend's side of the call: an impersonation-derived, short-lived ID token, an object name and a duration out, a verdict back — never a byte. |
| ✅ | `infrastructure/media-validator-iam.md` | [§3.1.1](https://secureacademic.com/gdpr-architectural-background/#sec-3-1-1) Media Integrity Validation | The two service accounts and the IAM Condition that make "isolated" a checkable claim rather than an adjective. |
...
- **"The upload never passes through the backend"** — read `signed-url-flow/issue-upload-url.js`: the request body it handles never contains file bytes, only a filename and content type. Note the scope of that claim: it is about the upload path, not about the object being unreachable in general. The same file now names the one remaining bounded case where the backend itself does read object content afterwards — the Proofreader's chunking step ([§5.3](https://secureacademic.com/gdpr-architectural-background/#sec-5-3)). The Transcriber's media-integrity check used to be a second such case; it no longer is — see the next point.
- **"The Transcriber's backend never sees the audio file, full stop"** — read `isolated-media-validator/index.js` and `isolated-media-validator/call-from-backend.js` together: the backend sends an object name and a duration, nothing else, and the only process that ever opens the object is a separate Cloud Run service running under its own IAM-restricted identity. `infrastructure/media-validator-iam.md` names exactly what that identity can, and cannot, reach.
